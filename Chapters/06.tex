...
\end{equation}
where $p_T$ is the transverse momentum of the Higgs, and $p_{i, T}$ is the transverse momentum of the $i$-th final state parton. The scale was chosen on the one hand to match previous studies~\cite{Lindert:2017pky, Bonciani:2022jmb, Jones:2018hbb}, but also to improve perturbative convergence at large $p_T$, where the Higgs mass alone is no longer the natural scale. This can for example be seen nicely in the phase-space measure in Eq.~\eqref{eq:4:PS_measure}, which shows that at large transverse momentum we encounter logarithms of the form $\log (\mu^2/s)$.
...
We estimate the scale uncertainties, based on the $\mu_R$ variation of the N${}^4$LO cross section in the soft-virtual approximation in the range $[m_H/4, m_H]$ presented in Ref.~\cite{Das:2020adl}. The factorization scale dependence was shown to be very small (see Fig.~\ref{fig:4:scale_scan}), making this an accurate estimate on missing higher order uncertainties. Uncertainties on the electroweak corrections are also dominated by missing higher order uncertainties and we adopt the uncertainties assigned in Ref.~\cite{Becchetti:2020wof}. Uncertainties associated to missing higher order in the \acs{PDF} fits, including the missing ingredients for the DGLAP evoluation, are estimated by comparing cross section results computed using the \texttt{NNPDF40\_an3lo\_as\_01180\_mhou} and the \texttt{NNPDF40\_an3lo\_as\_01180} \acs{PDF} set. This is done under the assumption that the \acs{PDF}-theory and the \acs{PDF} uncertainties are uncorrelated. Uncertainties for finite quark-mass effects are determined through seven-point scale variation.
\begin{equation}
\begin{alignedat}{5}
