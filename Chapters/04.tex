...
\begin{equation}
\mathcal{L}_{\text{HTL}}^{(0)} = \mathcal{L}_{\text{QCD}}^{(n_f - 1)} + C^{(0)} \frac{H}{v} \frac{1}{4} G^a_{\mu \nu} G^{a\, \mu\nu}.
...
\begin{equation}
\mathcal{L}_{\text{HTL}} = \mathcal{L}_{\text{QCD}}^{(5)} + \frac{\alphas}{\pi} \frac{H}{v} \sum_{i=1}^5 C_i^B \mathcal{O}_i^B.
...
\begin{equation}
\begin{split}
G_{A,A,\mathcal{O}_h}^{B,ab\, \mu\nu} (p, 0) \Big \vert_{\text{amp.}} &\simeq - \int \dd^d x\, \dd^d y\, e^{ip\cdot (x - y)} \braket{\Omega| T \left[ A^{B,a\mu}(x) A^{B, b \nu}(y) \frac{\alphas}{\pi} \frac{1}{m_h^B} \sum_{i=1}^5 C_i^B \mathcal{O}_i^B(0) \right]|\Omega} \Big \vert_{\text{amp.}}.
\end{split}
\end{equation}
...
With the effective theory matched, we are ready to discuss higher order corrections to the Higgs production cross section. Following the instructions of section~\ref{sec:2:cross_sections}, we start with the computation of the hard scattering amplitudes. At \acs{NLO} we require the one-loop Higgs-gluon form factor, and the tree-level amplitudes for $q \bar{q} \rightarrow H g$, $q g \rightarrow H q$, and $g g \rightarrow H g$.
