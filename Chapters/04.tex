...
Beyond \acs{LO}, gauge invariant operators like $\mathcal{O}_1$ can mix under renormalization with other gauge invariant operators but notably also with operators which are not gauge invariant. To see this, let us consider the following partition function:
\begin{equation}
\begin{gathered}
...
\end{gathered}
\end{equation}
Here, in addition to the usual fields and sources, we added a source $K$, which couples to the \textit{Becchi-Rouet-Stora} (BRS) variation of the gauge and ghost field. The latter is defined as
\begin{equation}
\begin{gathered}
...
\label{eq:4:sGamma}
\end{equation}
Here, we also defined the \textit{linearized Slavnov operator} $s$. Renormalization iteratively removes the divergences at each loop order, it is therefore straightforward to see that Eq.~\eqref{eq:4:sGamma} holds at every loop order. This equation tells us, that although the counter terms of the effective action are not required to be gauge invariant, they do need to be invariant under the action of $s$. Since counter terms are also polynomial in the external fields and have ghost number zero, one can then show (\textit{Becchi-Rouet-Stora-Tyutis} (BRST) \textit{cohomology})~\cite{Kluberg-Stern:1974iel, Joglekar:1975nu, Henneaux:2011rma} that the counter terms are linear combinations of BRST-exact, and gauge invariant operators
\begin{equation}
\hat{\Gamma}_{\text{ct}} = s F + \text{gauge invariant operators}.
\end{equation}

This equation may now be leveraged to determine a complete operator basis, as we shall demonstrate now. Since we are not interested in Green's functions of the BRS variation of fields, we can set $K^a_\mu = L^a = 0$ for all practical purposes. We can then make the general Ansatz
\begin{equation}
\begin{split}
...
& = \frac{\delta \hat{S}}{\delta A^a_\mu} \cdot \frac{\delta f}{\delta (\partial^\mu \bar{c}^a)} + \frac{\delta f}{\delta A^a_\mu} + \frac{\delta \hat{S}}{\delta L^a} \cdot \frac{\delta f}{\delta c^a} + \frac{\delta \hat{S}}{\delta c^a} g^a.
\end{split}
\label{eq:4:sF}
\end{equation}
...
\end{align*}
Note that $\delta/\delta (c^a)$ and $\delta/\delta(\partial_\mu \bar{c}^a)$ are \textit{Grassmann odd}, \ie\ we get an additional minus sign whenever we permute the derivatives with other ghost fields. This, for example explains the lack of a minus sign in $\delta S_{FP}/\delta c^a$. All derivatives not explicitly shown Eq.~\eqref{eq:4:functional_derivatives} vanish. These functional derivatives can now be inserted into Eq.~\eqref{eq:4:sF}, yielding
\begin{equation}
\begin{split}
...
\end{equation}
So up to a factor, the functions must be
\begin{equation}
...
\end{equation}
Additional gauge invariant operators like $\sum_i \bar{q}_i \slashed{D} q_i$ are now no longer independent can be expressed in terms other operators, in this by $\mathcal{O}_2$ and $\mathcal{O}_3$. Note also that the operator $\mathcal{O}_5$ is gauge invariant, vanishes by the classical equations of motion, and as we showed above is BRST-exact.

The BRST-transformation generated by the linearized Slavnov-operator is a symmetry of the action. The charge of the symmetry defines the physical space, \ie\ physical states are in the kernel of the charge operator. The matrix element of BRST-exact operators between physical states is vanishing.

In summary, we can distinguish between 3 types of operators: \textit{type-I operators}, which are gauge invariant and have non-zero matrix elements, \textit{type-II${}_a$ operators} which are gauge invariant but give zero matrix elements due to the equation of motion, and finally \textit{type-II${}_b$ operators} which are gauge variant but BRST-exact and hence also give zero between physical states.

In our example the complete operator basis reads
\begin{equation}
\begin{split}
...
\end{equation}
The operator basis is constructed only of light fields, and the light fields are defined in a decoupled theory. As we discussed before, the operator $\mathcal{O}_4$ is BRST-exact but also gauge invarinat and vanishes by the equations of motion. It thus classifies both as a type II${}_a$ and a type II${}_b$ operator.
