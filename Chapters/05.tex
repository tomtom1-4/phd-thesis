...
The amplitude is a function of four variables, \eg\ $s, t, u$ and the quark mass $m$. The Higgs mass is not a free parameter, as it can be expressed as a sum over the Mandelstams. We can however simply factor our one scale---say the collision scale $s$---to make everything dimensionless, and we are left with only three. We choose to parameterize the amplitude in terms of the dimensionless variables
\begin{equation}
...
\end{equation}
Once again, the exact structure of the one-loop coefficients $I_{ij}, R_{ij}$ is irrelevant for our purposes (see Ref.~\cite{Czakon:2015exa} for a definition). The eikonal factor is defined as
\begin{equation}
...
So far, our discussion of amplitudes was limited to \acs{OS}-renormalized quark masses. In the \acs{OS} scheme, the renormalized mass is identical to the pole mass, that means it corresponds to the position of the pole of the quark propagator. It is however not physical since quarks cannot exist as free asymptotic particles. Scattering matrices in \acs{QCD} will therefore not exhibit poles at the \acs{OS} mass. Nevertheless, we note that it is the \acs{OS} mass which enters the Yukawa coupling to the Higgs. With the inclusion of electroweak effects, the \MS\ renormalized Yukawa coupling will thus no longer be proportional to the \MS\ mass~\cite{Hempfling:1994ar}.
...
\end{equation}
The sum once again runs over all quark flavors decoupled from the running of $\alphas$, with masses less or equal than $m$. We once again underscore that these are not the ``standard'' relations between the pole and the \MS-masses, thanks to our prescription of the bottom quark mass in loops that do not couple to the Higgs. However, they follow upon replacing
\begin{equation}
...
\end{align}
The first two orders in the expansion already indicate a relatively good perturbative convergence for the top quark, whereas the bottom quark receives quite large correction, indicating poor convergence of the perturbartion series. At higher loop order, this phenomenon actually worsens~\cite{Marquard:2015qpa,Marquard:2016vmy}. Although there exists no formal proof in \acs{QCD}, it is believed that the perturbarion series in Eq.~\eqref{eq:5:mOS_mMS} only converges asymptotically. Asymptotic expansions can sometimes be summed by means of a \textit{Borel transform} defined by
\begin{equation}
f(\alphas) = \sum_{n = 1}^\infty c_n \alphas^n \quad \Rightarrow \quad B[f](t) = \sum_{n = 0}^n c_{n + 1} \frac{t^n}{n!}.
\end{equation}
The omission of the tree-level $\alphas^0$ term is conventional. Then, formally we find
\begin{equation}
f(\alphas) = \int_0^\infty \dd t\, e^{-\frac{t}{\alphas}} B[f](t),
\label{eq:5:fBorel_summed}
\end{equation}
in the sense that, this reproduces the expansion of $f$. This way, we can assign finite values to divergent series. However, if the Borel transform of the series has a pole on the real positive axis---say at $t_0 >0$---, then we cannot perform the integration in Eq.~\eqref{eq:5:fBorel_summed} right away. Even so, we can shift the pole away from the real axis by a small amount $\pm i 0^+$, so that we can define the finite integral
\begin{equation}
