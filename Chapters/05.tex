...
\begin{equation}
m^{\text{OS}}  = \overline{m}(\mu) \left(1 + c_1(\mu) \frac{\alphas(\mu)}{\pi} + c_2(\mu) \left( \frac{\alphas(\mu)}{\pi} \right)^2 + \mathcal{O}(\alpha^3)\right),
\label{eq:5:mOS_mMS}
\end{equation}
and then expand all amplitudes in $\alphas$ up to the required fix order. The relation between the quark masses in the different renormalization schemes can be extracted using the renormalization scheme invariance of the bare quark mass, which yields the relation
\begin{equation}
...
\begin{align*}
c_1(&\mu) = C_F \left(1 -\frac{3}{4} \ln\! \left(\frac{\overline{m}(\mu)^2}{\mu^2} \right) \right), \\
c_2(&\mu) = -\frac{1}{384} C_F \bigg \lbrace C_A \left(-132 \ln^2\!\left(\frac{\overline{m}(\mu)^2}{\mu^2} \right) +740 \ln\! \left(\frac{\overline{m}(\mu)^2}{\mu^2} \right) +144 \zeta (3)-1111+\pi ^2 \left( 32-96 \ln (2) \right) \right) \\
& \hspace{0.5cm} +3 C_F \bigg(-36 \ln^2\! \left(\frac{\overline{m}(\mu)^2}{\mu^2} \right) -36 \ln\! \left(\frac{\overline{m}(\mu)^2}{\mu^2} \right) -96 \zeta (3)+71+8 \pi ^2 (8 \ln (2)-5)\bigg) \\
& \hspace{0.5cm} +4 n_l T_F\left(12 \ln^2\! \left( \frac{\overline{m}(\mu)^2}{\mu^2} \right) -52 \ln\! \left( \frac{\overline{m}(\mu)^2}{\mu^2} \right) +8 \pi ^2+71\right) \bigg  \rbrace \\
& \hspace{-0.1cm}  -  \sum_i \frac{1}{96} C_F T_F \bigg(12 \ln^2\! \left( \frac{\overline{m}(\mu)^2}{\mu^2} \right) -24 \ln\! \left(\frac{\overline{m}(\mu)^2}{\mu^2} \right) \ln\! \left( \frac{\overline{m}_i(\mu)^2}{\mu^2} \right) -52 \ln\! \left(\frac{\overline{m}(\mu)^2}{\mu^2} \right) +32 \ln\!\left( \frac{\overline{m}_i(\mu)^2}{\mu^2} \right) \\
& \hspace{0.5cm} +48 (x_i-1)^2 \left(x_i^2+x_i+1\right) \text{Li}_2(1-x_i)-48
\left(x_i^4+x_i^3+x_i+1\right) \left(\text{Li}_2(-x_i) + \ln\!(x_i) \ln\!(x_i + 1) \right) \\
& \hspace{0.5cm} + 48 x_i^4 \ln ^2(x_i) + 48 x_i^2 \ln (x_i) - 16 \pi^2 (x_i^3 + x_i) +72 x_i^2 +71\bigg),
\end{align*}
\end{minipage}
}
\refstepcounter{equation}
...
\end{equation}
where we defined $ x_i = \frac{\overline{m}_i(\mu)}{\overline{m}(\mu)}$. The sum runs over all quark flavors which are decoupled from the running of $\alphas$. This includes the quark with mass $\overline{m}$ in case it is decoupled. This implies that the \acs{OS}-\MS-mass relation of the bottom-quark somehow depends on the mass of the top-quark. Since the top-quark mass is at much larger than the bottom-quark mass, this seems quite unphysical. The problem is that we relate the \acs{OS}-mass with the bottom-quark mass within the full $n_f = 6$ theory. Instead, we would like to relate the \acs{OS}-mass to the decoupled masses defined by
\begin{equation}
\begin{split}
\overline{m}^{(n_l)} &= \zeta_m\!\left(\alphas^{(n_l + 1)}(\mu) , \ln \frac{m^2}{\mu^2} \right) \alphas^{(n_l + 1)}(\mu), \\
...
\end{split}
\end{equation}
Ones we expand our relation in $x_i \ll 1$, which we do anyway, since all decoupling relations only hold up to power corrections, we find that all non-decoupling effects cancel, and our coefficients become
\begin{equation}
\resizebox{\textwidth}{!}{
\begin{minipage}{\textwidth}
\begin{align*}
c_1(&\mu) = C_F \left(1 -\frac{3}{4} \ln\! \left(\frac{\overline{m}(\mu)^2}{\mu^2} \right) \right), \\
c_2(&\mu) = -\frac{1}{384} C_F \bigg \lbrace C_A \left(-132 \ln^2\!\left(\frac{\overline{m}(\mu)^2}{\mu^2} \right) +740 \ln\! \left(\frac{\overline{m}(\mu)^2}{\mu^2} \right) +144 \zeta (3)-1111+\pi ^2 \left( 32-96 \ln (2) \right) \right) \\
& \hspace{2.5cm} +3 C_F \bigg(-36 \ln^2\! \left(\frac{\overline{m}(\mu)^2}{\mu^2} \right) -36 \ln\! \left(\frac{\overline{m}(\mu)^2}{\mu^2} \right) -96 \zeta (3)+71+8 \pi ^2 (8 \ln (2)-5)\bigg) \\
& \hspace{2.5cm} +4 n_l T_F\left(12 \ln^2\! \left( \frac{\overline{m}(\mu)^2}{\mu^2} \right) -52 \ln\! \left( \frac{\overline{m}(\mu)^2}{\mu^2} \right) +8 \pi ^2+71\right) \bigg \rbrace \\
& \hspace{0.9cm}  -  \frac{1}{96} C_F T_F \bigg( 71 - 20 \ln\! \left( \frac{\overline{m}(\mu)^2}{\mu^2} \right) - 12 \ln^2\! \left( \frac{\overline{m}(\mu)^2}{\mu^2} \right) + 8 \pi^2 \bigg).
\end{align*}
\end{minipage}
}
\refstepcounter{equation}
\tag{\theequation}
\end{equation}
Note that this does not work in case the mass hierarchy is inverted, \ie\ $x_i \gg 1$, for example when considering the top-quark \acs{OS}-\MS-mass relation in the 4\acs{FS}. Here, the finite effects of the light quark mass has to be incorporated using the full dependence in Eq.~\eqref{eq:5:c1_c2_full} or via an appropriate expansion in $1/x_i$.
...
\end{align}
We stress that the top-quark masses on the right-hand side of Eqs.~\eqref{eq:5:mtOS_mtMS_5FS} and \eqref{eq:5:mtOS_mtMS_4FS} are not decoupled, and for the bottom-quark masses in Eqs.~\eqref{eq:5:mbOS_mbMS_5FS} and\ \eqref{eq:5:mbOS_mbMS_4FS}, only the top-quark is decoupled. Furthermore, we once again underscore that these are not the standard relations between the pole and the \MS-masses, thanks to our prescription of the bottom quark mass in loops that do not couple to the Higgs. However, the standard relations follow from the first respective lines of Eqs.~\eqref{eq:5:mtOS_mtMS_4FS} and\ \eqref{eq:5:mbOS_mbMS_4FS} upon replacing $\alphas^{(4)}$ by $\alphas^{(5)}$. The first two expansion parameters already indicate a relatively good perturbative convergence for the top-quark, whereas the bottom quark receives quite large correction, a phenomenon which actually worsens with higher orders of perturbation theory~\cite{Marquard:2015qpa,Marquard:2016vmy}.
